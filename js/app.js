...
class PBook {
  ...
  async init() {
    ...
    if (inviteMatch && !localStorage.getItem('pbook-invite-used')) {
      ...
      if (/^E-/i.test(code)) {
        ...
        setTimeout(() => this.showXPToast('🛠 Vítej v redakčním týmu — svou konzoli najdeš v Profilu', 'achievement'), 1500);
      } else {
        this.rc.logEvent('invite_accepted', { code });
        if (this._f('gamification')) { this.user.addXP(25); this.user.save(); }
        setTimeout(() => this.showXPToast('🎁 +25 XP — pozval tě kamarád. Vítej!', 'achievement'), 1500);
      }
    }
    try {
      this.book = await (await fetch(CONFIG.book.bookIndex)).json();
    } catch (e) {
      document.body.innerHTML = '<div style="padding:3em;text-align:center;font-family:sans-serif;color:#888">Knihu se nepodařilo načíst. Nejdřív spusť <code>./serve.sh</code>.</div>';
      return;
    }
  ...
  _showRemixDecision(originalId, remixId, oldText, newText, isDiagram) {
    ...
    el.innerHTML = `
      <div style="font-weight:700">✨ ${isDiagram ? 'Nový diagram je hotový' : 'Přepis je hotový'} — zatím je to jen náhled, dokud se nerozhodneš.</div>
      ${diff}
      <div style="display:flex;gap:.45em;flex-wrap:wrap;margin-top:.35em">
        <button class="steer-chip" style="border-color:#10B981;color:#065F46;font-weight:700" onclick="app._decideRemix('${originalId}','${remixId}','accept')">✓ Přijmout</button>
        <button class="steer-chip" style="border-color:var(--accent);color:var(--accent);font-weight:700" onclick="app._decideRemix('${originalId}','${remixId}','share')">✓ Přijmout a sdílet</button>
        <button class="steer-chip" style="opacity:.85" onclick="app._decideRemix('${originalId}','${remixId}','discard')">✗ Zahodit</button>
      </div>`;
  ...
  async acceptRemix(originalId, remixId, share) {
    ...
    if (share) {
      ...
      this.showXPToast('+5 XP ✨ Přijato — vyber si jméno (nebo zůstaň v anonymitě) a sdílej', 'achievement');
    } else {
      this.showXPToast('+5 XP ✨ Tvoje verze je teď součástí tvé knihy (kdykoli ji můžeš vrátit)', 'achievement');
    }
  }
  ...
  discardOverride(originalId) {
    if (!window.confirm('Opravdu chceš navždy zahodit svou upravenou verzi této části? Tohle už nejde vrátit.')) return;
  ...
  _renderFeedbackBar(block) {
    ...
    return `<div class="steer-bar feedback-bar" id="steer-${block.id}" onpointerdown="app._pinFeedbackBar('${block.id}')">
      <button class="fb-close" onclick="app._dismissFeedbackBar('${block.id}')" title="Zavřít">&times;</button>
      <span class="steer-label">Jak ti tohle podání sedlo?</span>
      <button class="steer-chip" onclick="app.steerBlock('${block.id}','praise')" title="Tohle podání mi sedlo">&#128077; Super</button>
      ${canSimpler ? `<button class="steer-chip" onclick="app.steerBlock('${block.id}','simpler')" title="Stejná myšlenka, jednodušší podání">&#128315; Jednodušeji</button>` : ''}
      ${canDeeper ? `<button class="steer-chip" onclick="app.steerBlock('${block.id}','deeper')" title="Stejná myšlenka, víc do hloubky">&#128316; Do hloubky</button>` : ''}
      ${block.visuality !== 'visual-first' ? `<button class="steer-chip" onclick="app.steerBlock('${block.id}','visual')" title="Stejná myšlenka, víc obrázků">&#128444;&#65039; Víc vizuálně</button>` : ''}
      <select class="steer-chip steer-lens" onchange="if(this.value){app.steerBlock('${block.id}','lens',this.value);this.value=''}" title="Příklady z tvého světa">
        <option value="">&#127758; můj svět&hellip;</option>
  ...
  _stateBadge(meta) {
    ...
    if (meta.remixOf) {
      return s === 'community'
        ? '<span class="telling-badge" style="color:#D97706;border-color:#D97706">✨ remix čtenáře</span>'
        : '<span class="telling-badge" style="color:#D97706;border-color:#D97706">✨ tvůj remix</span>';
    }
    if (s === 'core') return '<span class="telling-badge" style="color:#7C3AED;border-color:#7C3AED">CORE</span>';
    if (s === 'community') return '<span class="telling-badge" style="color:#D97706;border-color:#D97706">⚡ sdílené čtenářem</span>';
    if (s === 'private') return '<span class="telling-badge" style="color:#D97706;border-color:#D97706">⚡ tvoje</span>';
    return '<span class="telling-badge" style="color:#10B981;border-color:#10B981">redakčně upravené</span>';
  }
  ...
  async toggleTellings(blockId, missMsg, presetTarget) {
    ...
    panel.innerHTML = '<div style="padding:.5em;color:var(--text-3);font-size:.75rem">Načítám podání…</div>';
    ...
    let h = `<div class="tellings-title">Podání konceptu &bdquo;${this.escHtml(concept?.title || conceptId)}&ldquo;</div>`;
    ...
    }).join('')}<span class="tstrip-hint">podání: ${pool.length} — najeď myší pro detail, klikni a čti</span></div>`;

    h += `<div class="tellings-compose">
      <div style="font-size:.72rem;font-weight:700;margin:.6em 0 .15em">🎛 Chceš to vyprávět jinak? <span style="font-weight:400;color:var(--text-3)">◉ = toto podání · <span class="legend-active">vyplněné</span> = tvůj cíl · čísla = existující podání${changed ? ` · <a href="#" onclick="event.preventDefault();app.resetPanelTarget('${blockId}')" style="color:var(--accent)">↺ vrátit (změněno: ${changed})</a>` : ''}</span></div>
    ...
    if (covered) {
      h += `<span>✓ Tvůj cíl už pokrývá: <b>${this.escHtml(best.b.meta.title || best.b.meta.id)}</b> (shoda ${Math.round(best.s * 100)} %)</span>
      ...
      h += `<span>Tohle zatím žádné podání nepokrývá — tvůj zájem jsme si zapsali a pomůže redakci rozhodnout, co psát dál. &#128203;</span>`;
    }
    h += '</div>';
    h += `<div style="font-size:.68rem;color:var(--text-3);margin-top:.35em">Tohle jsou různá podání jednoho konceptu. Chybí ti celý <i>koncept</i>? <a href="#" onclick="event.preventDefault();app.proposeConcept()" style="color:var(--accent)">🌱 navrhni ho</a> (+10 XP).</div>`;
    panel.innerHTML = h;
  }
  ...
  _renderDimValues(blockId, dim, pool, curMeta) {
    ...
    return conf.values.map(v => {
      ...
      return `<button class="steer-chip dimval ${isCur ? 'dim-current' : ''} ${isSel ? 'dim-active' : ''} ${count ? '' : 'dim-empty'}"
        onclick="app.steerDim('${blockId}','${dim}','${v}')" title="${isCur ? 'podání, které právě čteš, tohle pokrývá · ' : ''}${isSel ? 'je ve tvém cíli (klikni pro odebrání) · ' : ''}${count ? 'pokrývá: ' + count + ' podání' : 'zatím žádné podání — buď první'}">
  ...
  async steerBlock(blockId, action, value) {
    ...
    if (action === 'praise') {
      ...
      this.showXPToast('Zapsáno — chceš víc podání jako tohle 👍', 'achievement');
      return;
    }
    ...
    const askLabel = facet === 'lens' ? `${icons[value] || ''} ${value}` : action === 'visual' ? 'vizuálnější' : `${action === 'simpler' ? 'jednodušší' : 'hlubší'} (${target.depth})`;
  ...
  async _serveOrMiss(blockId, conceptId, target, candidateFilter, info) {
    ...
    if (best) {
      ...
      this.toggleTellings(blockId, `Podání <b>${info.askLabel}</b> tady zatím není — tohle už existuje:`, target);
  ...
  async _swapBlock(originalId, variantEntry, target, offerGenerate = false) {
    ...
    const noticeBits = [];
    if (vMeta.lens && vMeta.lens !== 'generic') noticeBits.push(`příklady: ${(CONFIG.facets.lens.icons || {})[vMeta.lens] || ''} ${vMeta.lens}`);
    if (vMeta.depth) noticeBits.push(`hloubka: ${vMeta.depth}`);
    const provenance = vMeta.state === 'community'
      ? `<span class="gen-badge">sdílel čtenář &middot; redakce zatím neověřila</span>`
      : vMeta.state === 'private'
        ? `<span class="gen-badge">&#9889; vygenerováno podle tvého nastavení &middot; redakce zatím neověřila</span>`
        : '';
    const notice = `<div class="variant-notice">
      ...
      <span>
        ...
        <button class="steer-chip" onclick="app.unswapBlock('${vMeta.id}')">&#8617; Původní</button>
      </span>
    </div>`;
  ...
  async generateVariant(blockId, conceptId) {
    ...
    try {
      ...
      if (!res.ok || !data.ok) throw new Error(data.error || 'generování selhalo');
      const block = {
      ...
      if (offer) offer.innerHTML = `<span>Generování se nepovedlo (${this.escHtml(e.message)}). Tvůj požadavek jsme zapsali pro redakci.</span>`;
  ...
  onboardPick(kind, value, el) {
    ...
    const WORLD = { generic: 'mixu různých platforem', ecommerce: 'e-shopů, košíků a „zákazníci také koupili“', media: 'playlistů, autoplaye a historie sledování', 'social-feeds': 'feedů, sledování a tvůrců', education: 'kurzů, cvičení a studentů' };
    const GOAL = { understand: 'hlavně jasná vysvětlení', build: 'praktické úkoly a technická hloubka, když budeš chtít', decide: 'důraz na výhody, nevýhody a rozhodování', protect: 'tvoje data, tvoje volby, tvoje práva' };
    const hint = document.getElementById('onboardHint');
    if (hint) hint.innerHTML = `→ Příklady budou z <b>${WORLD[lens]}</b> · ${GOAL[goal]}. Každou část si můžeš později doladit.`;
  }
  ...
  _customizeWelcome() {
    ...
    if (dueRecalls.length > 0) {
      html += `<button class="btn-secondary" style="font-size:.78rem;padding:.4em .8em;border-radius:8px;border:1.5px solid var(--warn);color:var(--warn);background:var(--warn-bg)" onclick="app.startAndGo('quiz');setTimeout(()=>app.startPractice(true),300)">\u{1F9E0} K opakování: ${dueRecalls.length}</button>`;
    } else if (Object.keys(u.recall).length > 0) {
      html += `<button class="btn-secondary" style="font-size:.78rem;padding:.4em .8em;border-radius:8px;border:1.5px solid var(--accent);color:var(--accent)" onclick="app.startAndGo('quiz')">\u{1F9E0} Otestuj se</button>`;
    }
    ...
    // Progress summary
    html += `<div style="width:100%;font-size:.7rem;color:var(--text-3);text-align:center;margin-top:.2em">${prog.read}/${prog.total} částí &middot; ${u.xp} XP &middot; úroveň ${u.level}</div>`;
  ...
  async renderHome() {
    ...
    const continueBlock = this.getContinueBlock();
    if (continueBlock) {
      html += this.shelf('Pokračuj ve čtení', [this.cardHtml(continueBlock, true)]);
    }
    ...
    // Recently added — new content (publishedAt within 30 days), local only
    {
      const newBlocks = this.allBlocks
        ...
        .slice(0, 12);
      if (newBlocks.length) {
        html += this.shelf('Nově přidané', newBlocks.map(b => this.cardHtml(b.meta)));
      }
    }
    ...
    if (forYou?.recomms?.length) {
      ...
      if (forYouCards.length) html += this.shelf('Vybráno pro tebe', forYouCards);
    }
        ...
        if (shared.length) {
          const cards = shared.map(b => {
            ...
            return `<div class="card" style="border-top:3px solid var(--warn,#D97706);flex:0 0 240px;cursor:pointer" onclick="app.openCommunityBlock('${this.escHtml(m.id)}')">
              <div class="card-chapter" style="color:var(--warn,#D97706);font-weight:700">⚡ vytvořil čtenář</div>
              <div class="card-title">${this.escHtml(m.title || m.id)}</div>
              <div class="card-teaser" style="font-size:.7rem">${facetLine ? this.escHtml(facetLine) + ' · ' : ''}sdílí ${this.escHtml(m.sharedAs || 'čtenář')}</div>
            </div>`;
          });
          html += this.shelf('Od ostatních čtenářů 🌱', cards);
        }
    ...
    if (ghosts.length) {
      html += this.shelf('Máme tohle napsat? 🌱 Hlasuj o návrzích',
    ...
    if (!isDefaultTarget) {
      const scored = this.allBlocks
        ...
        .sort((x, y) => y.s - x.s)
        .slice(0, 10);
      if (scored.length >= 3) html += this.shelf('🎛 Vyprávěno po tvém', scored.map(x => this.cardHtml(x.b.meta)));
    }
    ...
    if (liked.length) {
      html += this.shelf('&#10084; Co se ti líbilo', liked.reverse().map(b => this.cardHtml(b.meta)));
    }

    // 6. Saved items (if any)
    const saved = [...this.user.savedBlocks].map(id => this.findBlock(id)).filter(Boolean);
    if (saved.length) {
      html += this.shelf('&#128278; Uloženo na později', saved.reverse().map(b => this.cardHtml(b.meta)));
    }
    ...
    this.book.chapters.forEach((ch, i) => {
      ...
      if (chBlocks.length) {
        html += this.shelf(`Kap. ${ch.number}: ${ch.title}`, chBlocks.map(b => this.cardHtml(b)));
      }
    });

    el.innerHTML = html || '<div class="search-empty">Načítám obsah…</div>';
  ...
  cardHtml(block, hero = false) {
    ...
    const newBadge = isNew ? '<span class="card-badge-new">NOVÉ</span>' : '';
    ...
    if (hasDiagram || hasMath || hasTable || hasCode) {
      ...
      if (hasMath) tags.push('<span class="card-tag tag-math">&#8721; Vzorce</span>');
      if (hasTable) tags.push('<span class="card-tag tag-table">&#9638; Tabulka</span>');
      if (hasCode) tags.push('<span class="card-tag tag-code">&lt;/&gt; Kód</span>');
  ...
  async renderSpine(block) {
    ...
    if (ovId && this.privateBlocks?.[ovId]) {
      ...
      if (this._overridesOff().has(block.id)) {
        ...
        overrideBar = `<div class="override-bar">📄 Původní verze — tvoje úprava je uložená
          · <a href="#" onclick="event.preventDefault();app.toggleOverride('${block.id}')">✨ zobrazit tvou verzi</a>
          · <a href="#" style="color:#B91C1C" onclick="event.preventDefault();app.discardOverride('${block.id}')">🗑 zahodit tvé úpravy (navždy)</a></div>`;
      } else {
        block = { ...block, body: ov.body, diagramSvg: ov.meta.diagramSvg || block.diagramSvg };
        overrideBar = `<div class="override-bar">✨ Tvoje přijatá verze — změny jsou <mark class="remix-mark">zvýrazněné</mark>
          · <a href="#" onclick="event.preventDefault();app.toggleOverride('${block.id}')">↩ zobrazit původní</a>
    ...
    if (bodyHtml.includes('⟦rx⟧')) {
      ...
      bodyHtml = bodyHtml.replace(/⟦rx⟧/g, '<mark class="remix-mark" title="Remixovaná pasáž — liší se od originálu">')
                         .replace(/⟦\/rx⟧/g, '</mark>');
    }
    ...
    return `<article class="block-article fade-up" id="b-${block.id}">
      ...
      ${block.keyTakeaway ? `<div class="key-takeaway"><div class="key-takeaway-label">To hlavní</div><div class="key-takeaway-text">${block.keyTakeaway}</div></div>` : ''}
      ...
      <div class="block-footer">
        ...
        <div class="block-actions">
          ...
          <button class="act-btn flag-btn" onclick="app.flagBlock('${block.id}')" title="Poslat návrh autorům">&#9873;</button>
        </div>
      </div>
      <div class="note-editor" id="note-${block.id}" style="display:none">
        ...
        <textarea placeholder="Tvoje poznámka k této části…" id="note-text-${block.id}"></textarea>
      ...
      <div class="flag-form" id="flag-${block.id}" style="display:none">
        <div class="flag-header">Poslat návrh autorům</div>
        <select id="flag-type-${block.id}">
          ...
          <option value="incorrect">Věcná chyba</option>
          <option value="suggestion">Návrh na obsah</option>
          <option value="missing">Něco tu chybí</option>
        </select>
        <textarea id="flag-text-${block.id}" placeholder="Tvůj postřeh (soukromý, uvidí ho jen autoři)…"></textarea>
        <div class="note-actions">
  ...
  saveNote(blockId) {
    ...
    if (editIdx >= 0 && editIdx < notes.length) {
      ...
      if (this._f('gamification')) { this.user.addXP(3); this.user.save(); this.showXPToast('+3 XP za poznámku'); this.updateXPBadge(); }
    }
    this._saveNotes(blockId, notes);
  ...
  _renderSideNote(blockId, n) {
    ...
    if (n.type === 'user') {
      ...
      return `<div class="snote snote-user"${clickAttr}><span class="snote-icon">${icon}</span><div class="snote-body">${quoteHtml}${noteHtml}</div><button class="snote-edit" onclick="event.stopPropagation();app.editUserNote('${blockId}',${n.idx})">upravit</button><button class="snote-del" onclick="event.stopPropagation();app.deleteUserNote('${blockId}',${n.idx})">&times;</button></div>`;
    }
  ...
  submitFlag(blockId) {
    ...
    if (form) {
      ...
      form.innerHTML = '<div style="padding:.8em;color:var(--product);font-size:.85rem">Díky! +5 XP — nahlášené chyby se počítají do tvé 🛠 redaktorské dráhy.</div>';
  ...
  highlightSelection() {
    ...
    if (blockId) {
      ...
      if (this._f('gamification')) { this.user.addXP(1); this.user.save(); this.showXPToast('+1 XP za zvýraznění'); this.updateXPBadge(); }
    }
    sel.removeAllRanges();
  ...
  renderGame(block) {
    ...
    return `<div class="game-block fade-up" id="b-${block.id}">
      ...
      <button class="game-start-btn" onclick="app.startGame('${block.id}','${gameFile}')">Hrát!</button>
    </div>`;
  }
  ...
  async startGame(blockId, gameFile) {
    ...
    try {
      ...
      area.innerHTML = '<div class="game-over">Hru se nepodařilo načíst.</div>';
      return;
    }
    ...
    this._activeGameTimer = setInterval(() => {
      ...
      if (seconds <= 0) {
        ...
        this._gameEnd(area, 'Čas vypršel! Dobrý pokus.');
      }
    }, 1000);
  ...
  _gameSort(area, game) {
    ...
    let score = 0, idx = 0;
    const show = () => {
      if (idx >= items.length) { this._gameEnd(area, `Hotovo! Správně: ${score}/${items.length}.`); return; }
      ...
      area.innerHTML = `<div class="game-signal-card">${item.text}</div>
        ...
        <div class="game-score">Správně: ${score}/${idx - 1}</div>`;
  ...
  _gameMatch(area, game) {
    ...
    table += `<tr class="game-you"><td><b>Ty</b></td>${you.map(r => `<td>${'\u2B50'.repeat(r)}</td>`).join('')}</tr>`;
    ...
    area.querySelectorAll('.game-pick').forEach((td, i) => {
      td.onclick = () => {
        if (i === twin) { this._gameEnd(area, `Správně! ${users[twin].name} má stejný vkus jako ty! Přesně takhle hledají podobné lidi doporučovací systémy.`); }
  ...
  _gamePairs(area, game) {
    ...
    let score = 0, idx = 0;
    const show = () => {
      if (idx >= pairs.length) { this._gameEnd(area, `Hotovo! Správně: ${score}/${pairs.length}.`); return; }
      const cur = pairs[idx++];
      ...
      area.innerHTML = `<div class="game-signal-card">${cur.b}</div>
        <div class="game-prompt" style="margin:.3em 0">${game.instruction || 'Který pojem to je?'}</div>
  ...
  _gamePop(area, game) {
    ...
    const render = () => {
      area.innerHTML = `<div class="game-prompt">${game.instruction} Tvoje bublina: <b>${target}</b> (${popped.size}/${cats.length - 1})</div>
      ...
      area.querySelectorAll('.game-bubble-item:not([disabled])').forEach(btn => {
        btn.onclick = () => {
          ...
          else { popped.add(btn.textContent.trim()); if (popped.size >= cats.length - 1) this._gameEnd(area, 'Bublina praskla! Pestrost vítězí!'); else render(); }
        };
      });
    };
    render();
      ...
      area.querySelectorAll('.game-pipe-btn').forEach(btn => {
        btn.onclick = () => {
          ...
          if (step && step.order === selected.length) {
            selected.push(step);
            if (selected.length >= steps.length) this._gameEnd(area, 'Perfektní pořadí! Zvládl/a jsi to!');
            else render();
  ...
  renderReadNext(blockId, ch) {
    ...
    if (similar.length) {
      items += '<div class="rn-similar"><div class="rn-similar-label">Podobné tomuhle</div>';
      similar.forEach(s => {
    ...
    if (this._sharedBlockId === blockId) {
      items += `<div class="rn-cta">
        <div class="rn-cta-text">Líbilo se ti to? Čeká na tebe dalších ${this.allBlocks.filter(b => b.meta.type === 'spine').length} částí!</div>
  ...
  shareBlock(blockId) {
    ...
    navigator.clipboard.writeText(url).then(() => {
      ...
      prompt('Zkopíruj tento odkaz:', url);
    });
  ...
  shareMission(missionId) {
    ...
    navigator.clipboard.writeText(url).then(() => {
      this.showXPToast('Odkaz zkopírován!', 'info');
    }).catch(() => { prompt('Zkopíruj tento odkaz:', url); });
    ...
    panel.innerHTML = `
      <div class="preview-content">
        ...
        <div class="preview-actions">
          ...
          <button class="preview-dismiss" onclick="document.getElementById('previewPanel').remove()">Zůstat tady</button>
        </div>
      </div>
    `;
  ...
  showBlockRecall(blockId) {
    ...
    const html = `<div class="inline-recall fade-up" id="block-recall-${blockId}">
      <div class="ir-header"><span class="ir-icon">\u{1F9E0}</span> Otestuj svou paměť</div>
      ...
      <div class="ir-answer" id="br-a-${blockId}" style="display:none">
        ...
        </div>` : `<button class="recall-reveal" onclick="document.getElementById('block-recall-${blockId}').remove()">Jasně!</button>`}
      </div>
  ...
  _insertInlineRecall(justReadId) {
    ...
    const html = `<div class="inline-recall fade-up" id="inline-recall-${r.blockId}">
      ...
      <div class="ir-answer" id="ir-a-${r.blockId}" style="display:none">
        <div class="ir-answer-text">${quiz.a}</div>
        <div class="ir-from">Z části: ${block.meta.title}</div>
  ...
  _recallCardHtml(r) {
    ...
    return `<div class="card recall-card" style="border-top: 3px solid var(--warn); flex: 0 0 280px">
      ...
      <div class="recall-answer" id="recall-a-${r.blockId}" style="display:none">
        ...
        <div class="recall-hint" style="font-size:.7rem;color:var(--text-3);margin:.3em 0">Z části: ${block.meta.title}</div>
        <div class="recall-buttons">
          <button class="recall-btn recall-forgot" onclick="app.scoreRecall('${r.blockId}',0,this)">Zapomněl/a</button>
          <button class="recall-btn recall-hard" onclick="app.scoreRecall('${r.blockId}',1,this)">Těžké</button>
          <button class="recall-btn recall-good" onclick="app.scoreRecall('${r.blockId}',2,this)">Dobře</button>
  ...
  _generateQuiz(block) {
    ...
    // --- Kid-friendly quizzes matched to (Czech) content keywords ---

    // Ch1: Jak vypadá internet
    if (body.includes('server')) quizzes.push({ q: 'Co je to server?', a: 'Počítač, který je pořád zapnutý a čeká na požadavky ostatních. Když otevřeš stránku, tvůj prohlížeč o ni požádá server — a ten mu ji pošle!' });
    if (body.includes('paket')) quizzes.push({ q: 'Co se stane se zprávou, než se pošle přes internet?', a: 'Rozseká se na malé kousky — pakety. Každý cestuje sítí sám (klidně jinou cestou) a v cíli se zase složí dohromady!' });
    if (body.includes('router') || body.includes('směrovač')) quizzes.push({ q: 'Co dělá router?', a: 'Funguje jako dopravní křižovatka: podívá se na adresu paketu a pošle ho dál tím správným směrem k cíli.' });
    if (body.includes('kabel') || body.includes('optick')) quizzes.push({ q: 'Jak se data dostanou přes oceán?', a: 'Podmořskými optickými kabely! Na dně oceánů leží stovky kabelů, kterými letí data rychlostí světla.' });
    if (body.includes('datacentr') || body.includes('cloud')) quizzes.push({ q: 'Kde doopravdy jsou tvoje fotky, když jsou „v cloudu“?', a: 'Na discích v obřích halách plných serverů — datacentrech. Cloud není mrak, ale něčí počítač!' });

    // Ch2: Jak se připojujeme
    if (body.includes('wi-fi') || body.includes('wifi')) quizzes.push({ q: 'Jak se liší Wi-Fi od mobilních dat?', a: 'Wi-Fi tě spojí s routerem doma nebo ve škole rádiovými vlnami na krátkou vzdálenost. Mobilní data jdou přes vysílače operátora — fungují skoro všude.' });
    if (body.includes('ip adres')) quizzes.push({ q: 'K čemu slouží IP adresa?', a: 'Je to adresa zařízení v síti — díky ní pakety vědí, kam mají doletět. Něco jako adresa domu pro pošťáka.' });
    if (body.includes('dns') || body.includes('domén')) quizzes.push({ q: 'Co udělá DNS, když napíšeš adresu stránky?', a: 'Přeloží jméno (třeba skola.cz) na IP adresu serveru. Je to takový telefonní seznam internetu!' });
    if (body.includes('signál') || body.includes('vysílač')) quizzes.push({ q: 'Proč je někde slabý signál?', a: 'Signál slábne se vzdáleností od vysílače a překážkami — zdmi, kopci, i počtem lidí, kteří síť zrovna používají.' });

    // Ch3: Digitální stopa
    if (body.includes('stopa') || body.includes('stopy')) quizzes.push({ q: 'Co je to digitální stopa?', a: 'Všechno, co po tobě na internetu zůstává: příspěvky, komentáře, lajky, ale i to, co o tobě sbírají weby na pozadí.' });
    if (body.includes('cookie') || body.includes('cookies')) quizzes.push({ q: 'K čemu slouží cookies?', a: 'Malé soubory, které si web uloží do tvého prohlížeče — aby si tě pamatoval. Hodí se na přihlášení, ale umí tě i sledovat napříč weby.' });
    if (body.includes('heslo') || body.includes('hesla')) quizzes.push({ q: 'Jaké je opravdu silné heslo?', a: 'Dlouhé (klidně celá věta), jedinečné pro každou službu a nikomu ho neříkáš. Krátká hesla počítač uhodne za pár sekund!' });
    if (body.includes('šifrov')) quizzes.push({ q: 'Co znamená, že je spojení šifrované?', a: 'Data se zamknou tak, že je po cestě nikdo nepřečte — rozluštit je umí jen příjemce. Pozná se to třeba podle zámečku u adresy.' });
    if (body.includes('sdíl')) quizzes.push({ q: 'Dá se z internetu něco úplně smazat?', a: 'Skoro nikdy! Co jednou sdílíš, může být zkopírované, uložené nebo vyfocené. Proto se před sdílením vždycky na chvilku zamysli.' });

    // Fallback
    if (quizzes.length === 0)
      quizzes.push({ q: 'Dokážeš „' + (block.title || 'tohle téma') + '“ vysvětlit kamarádovi jednou větou?', a: 'Zkus to! Když to umíš říct jednoduše, opravdu tomu rozumíš. Když ne, přečti si část ještě jednou — napodruhé to dává větší smysl!' });

    const quiz = quizzes[Math.floor(Math.random() * quizzes.length)];
    return `<h4>&#129504; Rychlý kvíz!</h4>
      <div class="ctx-quiz">
        <div class="ctx-quiz-q">${quiz.q}</div>
        <button class="ctx-quiz-reveal" onclick="this.nextElementSibling.style.display='block';this.style.display='none'">Hmm, chvilku přemýšlím… &#129300; Ukázat odpověď!</button>
  ...
  _getRecallQuestion(block) {
    ...
    if (sentences.length >= 2) {
      ...
      return { q: `Co ses dozvěděl/a v části „${title}“?`, a: answer + '.' };
    }

    return { q: `Jaká je hlavní myšlenka části „${title}“?`, a: `Vzpomeň si, co tahle část vysvětlovala. Zkus si „${title}“ přečíst znovu, ať se ti to připomene!` };
  }
  ...
  renderQuiz() {
      ...
      if (block) {
        ...
        if (quiz) {
          h += `<div style="padding:1em;max-width:500px;margin:0 auto">
            <p style="font-size:.75rem;color:var(--text-3);margin-bottom:.8em">Někdo s tebou sdílel tuhle otázku:</p>
            <div class="recall-card-big">
              ...
              <div id="shared-a" style="display:none">
                ...
                <div style="margin-top:.5em;font-size:.72rem;color:var(--text-3)">Z části: kap. ${block.meta._chapterNum} — ${block.meta.title}</div>
                <div style="display:flex;gap:.4em;margin-top:.6em">
                  <button class="btn-primary" style="flex:1;font-size:.78rem" onclick="app.openBlock('${sharedId}')">Přečíst tuhle část</button>
    ...
    h += `<div style="padding:.8em 1em .2em">
      <h2 style="font-family:var(--font-ui);font-size:1.15rem;font-weight:800">\u{1F9E0} Otestuj své znalosti</h2>
      <p style="font-size:.75rem;color:var(--text-3);margin-top:.1em">Chytré opakování — zopakuj si, co ses naučil/a</p>
    </div>`;

    if (totalRead === 0) {
      h += `<div style="text-align:center;padding:3em 1em;color:var(--text-3)">
        <p style="font-size:2rem;margin-bottom:.3em">\u{1F4DA}</p>
        <p style="font-size:.9rem;font-weight:600">Nejdřív si něco přečti!</p>
        <p style="font-size:.78rem;margin:.3em 0 1em">Paměťové kartičky se vytvářejí automaticky při čtení.</p>
    ...
    if (due.length > 0) {
      const dueCards = due.slice(0, 10).map(r => {
        ...
        const reason = overdue > 24 ? `zpoždění ${Math.round(overdue/24)} d` : overdue > 0 ? `zpoždění ${overdue} h` : 'Teď na řadě';
        return `<div class="card recall-card" style="border-top:3px solid var(--warn);flex:0 0 280px">
          ...
          <div id="qd-a-${r.blockId}" style="display:none">
            ...
            <div style="font-size:.6rem;color:var(--text-3);margin-bottom:.3em">Z části: ${block.meta.title}</div>
            <div class="recall-buttons">
              <button class="recall-btn recall-forgot" onclick="app.scoreRecall('${r.blockId}',0,this)">Zapomněl/a</button>
              <button class="recall-btn recall-hard" onclick="app.scoreRecall('${r.blockId}',1,this)">Těžké</button>
              <button class="recall-btn recall-good" onclick="app.scoreRecall('${r.blockId}',2,this)">Dobře</button>
      ...
      if (dueCards.length) h += this.shelf(`\u{1F525} Teď na řadě (${due.length})`, dueCards);
    }
    ...
    const allCards = [
      ...hardCards.map(([id]) => ({ id, color: '#dc2626', label: 'S obtížemi', q: qFor(id) })),
      ...newCards.map(id => ({ id, color: 'var(--accent)', label: 'Nové', q: qFor(id) })),
      ...medCards.map(([id]) => ({ id, color: 'var(--warn)', label: 'Učím se', q: qFor(id) })),
      ...easyCards.map(([id]) => ({ id, color: 'var(--product)', label: 'Jistota', q: qFor(id) })),
      ...unreadBlocks.map(b => ({ id: b.meta.id, color: 'var(--border)', label: 'Nepřečteno', q: b.meta.title })),
    ].filter(c => c.q);
    if (allCards.length > 0) {
      ...
      h += `<div style="padding:.5em 1em .6em">
        <div style="display:flex;flex-wrap:wrap;gap:2px" id="quizMap">
          ${allCards.map(c => `<div style="width:${cellW}px;height:${cellW}px;border-radius:2px;background:${c.color};cursor:pointer;transition:transform .1s" title="${this.escHtml(c.label + ': ' + c.q)}" onclick="app.${c.label === 'Nepřečteno' ? "openBlock('" + c.id + "')" : "showBlockRecall('" + c.id + "')"}"></div>`).join('')}
        </div>
        <div style="display:flex;gap:.8em;font-size:.58rem;color:var(--text-3);margin-top:.4em">
          ...
          ${newCards.length ? `<span><span style="display:inline-block;width:8px;height:8px;border-radius:2px;background:var(--accent);vertical-align:middle"></span> ${newCards.length} nových</span>` : ''}
          ${medCards.length ? `<span><span style="display:inline-block;width:8px;height:8px;border-radius:2px;background:var(--warn);vertical-align:middle"></span> ${medCards.length} v učení</span>` : ''}
          ${easyCards.length ? `<span><span style="display:inline-block;width:8px;height:8px;border-radius:2px;background:var(--product);vertical-align:middle"></span> ${easyCards.length} s jistotou</span>` : ''}
          ${unreadBlocks.length ? `<span><span style="display:inline-block;width:8px;height:8px;border-radius:2px;background:var(--border);vertical-align:middle"></span> ${unreadBlocks.length} nepřečtených</span>` : ''}
        </div>
      </div>`;
    }
    ...
    if (due.length > 0) {
      const dueCards = due.slice(0, 12).map(r => {
        ...
        const card = u.recall[r.blockId];
        const overdue = Math.round((Date.now() - card.nextReview) / 3600000);
        const reason = overdue > 24 ? `zpoždění ${Math.round(overdue/24)} d` : overdue > 0 ? `zpoždění ${overdue} h` : 'Na řadě';
        return this._quizPreviewCard(r.blockId, card, 'var(--warn)', reason);
      }).filter(Boolean);
      if (dueCards.length) h += this.shelf(`\u{1F525} Zopakuj teď (${due.length})`, dueCards);
    }
    ...
    if (newCards.length > 0) {
      ...
      if (nCards.length) h += this.shelf(`\u{1F7E3} Nové — zatím netestováno (${newCards.length})`, nCards);
    }
    ...
    if (easyCards.length > 0) {
      const eCards = easyCards.sort((a,b) => b[1].ease - a[1].ease).slice(0, 10).map(([id, c]) => this._quizPreviewCard(id, c, 'var(--product)', 'Jistota')).filter(Boolean);
      if (eCards.length) h += this.shelf(`\u{1F7E2} Jistota (${easyCards.length})`, eCards);
    }
    ...
    if (unreadBlocks.length > 0) {
      const uCards = unreadBlocks.slice(0, 8).map(b => {
        return `<div class="card" style="border-top:3px solid var(--border);flex:0 0 240px;opacity:.7;cursor:pointer" onclick="app.openBlock('${b.meta.id}')">
          <div style="font-size:.6rem;font-weight:700;color:var(--text-3);margin-bottom:.2em">\u{1F512} Zatím nepřečteno</div>
          <div class="card-title" style="font-size:.82rem;line-height:1.3">${b.meta.title}</div>
          <div style="font-size:.62rem;color:var(--text-3);margin-top:.2em">Kap. ${b.meta._chapterNum} · Přečti a odemkni kartičku</div>
        </div>`;
      });
      h += this.shelf(`\u{26AA} Zatím nepřečtené (${unreadBlocks.length})`, uCards);
    }
      ...
      <details style="font-size:.75rem;color:var(--text-2)">
        ...
        <div style="margin-top:.5em;line-height:1.5">
          ...
          <ul style="padding-left:1.2em;margin:.4em 0">
            <li><strong>Zapomněl/a</strong> → kartička se přesune do S obtížemi a brzy se zopakuje</li>
            <li><strong>Těžké</strong> → zůstane v aktuálním pruhu, kratší interval</li>
            <li><strong>Dobře</strong> → posune se směrem k Jistotě, normální interval</li>
            <li><strong>Snadné</strong> → přesune se do Jistoty, delší interval</li>
          </ul>
          <p>I jisté kartičky se vracejí — bez opakování znalosti blednou. Algoritmus nejčastěji testuje středně těžké kartičky (na nich se naučíš nejvíc).</p>
        </div>
      </details>
    </div>`;
    ...
    if (upcoming.length > 0) {
      h += `<div style="padding:.5em 1em">
        <div style="font-size:.7rem;font-weight:600;color:var(--text-3);margin-bottom:.3em">Nejbližší opakování</div>
        ${upcoming.map(([id, c]) => {
    ...
    h += `</div>`;
    if (hardCards.length > 0) {
      h += `<div style="text-align:center;margin-top:.4em"><button style="border:1.5px solid #dc2626;border-radius:8px;padding:.35em .8em;font-size:.72rem;color:#dc2626;cursor:pointer;background:none;font-family:var(--font-ui)" onclick="app._startHardMode()">Těžký režim (${hardCards.length})</button></div>`;
    }
    h += `</div>`;
  ...
  _quizPreviewCard(blockId, card, color, label) {
    ...
    const timeLabel = card.nextReview ? (isDue ? 'teď na řadě' : this._timeUntil(card.nextReview)) : '';
    ...
    return `<div class="card" style="border-top:3px solid ${color};flex:0 0 260px;cursor:pointer" onclick="var a=document.getElementById('qp-${uid}');if(a)a.style.display=a.style.display==='none'?'block':'none'">
      ...
      <div id="qp-${uid}" style="display:none;margin-top:.4em;padding-top:.4em;border-top:1px solid var(--border)">
        ...
        <a href="#" onclick="event.stopPropagation();event.preventDefault();app.openBlock('${blockId}')" style="display:block;font-size:.62rem;color:var(--accent);margin-top:.3em;text-align:center">Přečíst znovu &rarr;</a>
      </div>
    </div>`;
  }
  ...
  _renderQuizCard() {
    ...
    if (idx >= q.length) {
      ...
      el.innerHTML = `<div class="recall-session">
        ...
        <h2>${perfect ? 'Plný počet!' : pct >= 70 ? 'Skvělá práce!' : 'Jen tak dál!'}</h2>
        ...
        <div style="display:flex;flex-direction:column;gap:.5em;margin-top:1em;align-items:center">
          ${unread.length > 0 ? `<button class="btn-primary" style="width:100%;max-width:280px" onclick="app.switchView('home')">\u{1F4D6} Číst nové části (zbývá ${unread.length})</button>` : ''}
          <button class="btn-ghost" style="border:1px solid var(--accent);border-radius:8px;padding:.5em 1em;font-size:.82rem;color:var(--accent);width:100%;max-width:280px" onclick="app.startPractice()">Otestovat další kartičky</button>
    ...
    el.innerHTML = `<div class="recall-session">
      <div class="recall-progress-row">
        ...
        ${item.isDue ? '<span class="recall-due-badge">Na řadě</span>' : ''}
      ...
      <div class="recall-card-big">
        ...
        <div class="recall-card-a" id="recallAnswer" style="display:none">
          ...
          <div style="display:flex;justify-content:space-between;align-items:center;margin:.4em 0">
            ...
            <span style="font-size:.6rem;color:var(--text-3)">opakováno ${reps}x &middot; obtížnost ${ease}</span>
          </div>
          <div class="recall-buttons">
      ...
      <div style="display:flex;justify-content:center;margin-top:.8em">
        <button style="font-size:.72rem;color:var(--text-3);border:1px solid var(--border);border-radius:6px;padding:.3em .8em;cursor:pointer" onclick="app._endPractice()">Ukončit (${this._recallScore.correct}/${idx} správně)</button>
      </div>
    </div>`;
    window.scrollTo(0, 0);
  }
  ...
  _answerRecall(blockId, quality) {
    ...
    if (quality === 0) {
      ...
      if (answerEl) {
        answerEl.insertAdjacentHTML('beforeend', `<div style="text-align:center;margin-top:.5em"><a href="#" onclick="event.preventDefault();app.openBlock('${blockId}')" style="color:var(--accent);font-size:.82rem;font-weight:600">Přečíst znovu &rarr;</a></div>`);
      }
      this._recallIdx++;
  ...
  async _renderCoverageMap() {
    ...
    const DIM_LABELS = { lens: '🌐 Svět', depth: '📏 Hloubka', visuality: '🖼 Forma', carriers: '🧩 Prvky', genre: '✒️ Žánr', lang: '🌍 Jazyk', lengthBand: '⏱ Délka' };
    ...
    const rowHtml = (meta, extraBadge) => {
      ...
      const cells = values.map(v => {
        ...
        return set.includes(v)
          ? `<td class="covmap-cell${my}" onclick="app.openTelling('${meta.id}')" title="pokrývá ${v} — číst"><span style="color:${color};font-size:.85rem">●</span></td>`
          : `<td class="covmap-cell${my}" style="cursor:default"><span style="color:var(--border)">·</span></td>`;
      }).join('');
      return `<tr><td class="covmap-concept" onclick="app.openTelling('${meta.id}')" title="Číst">
    ...
    for (let i = 0; i < this.book.chapters.length; i++) {
      ...
      extras.forEach(({ meta, kind }) => {
        rows += rowHtml(meta, `<span class="telling-badge" style="color:#D97706;border-color:#D97706">${kind === 'yours' ? '✨ tvoje' : '⚡ sdílené'}</span> `);
      });
  ...
  covInspect(conceptId, value) {
    ...
    let h = `<div class="cov-inspector">
      <div class="tellings-title" style="display:flex;justify-content:space-between;align-items:center">
        ...
        <button class="steer-chip" onclick="document.getElementById('covInspector').innerHTML=''">✕ zavřít</button>
      </div>`;
    if (!inCell.length) {
      h += `<div class="tellings-miss">Tady zatím nic není — tohle políčko je pozvánka.</div>
        <button class="steer-chip steer-gen" onclick="app.coverageGap('${conceptId}','${value}')">Otevřít koncept a vyžádat / vygenerovat tohle podání →</button>`;
    } else {
      h += inCell.map(({ b }) => `<div class="telling-row">
          ...
          <button class="steer-chip" onclick="app.openTelling('${b.meta.id}')">Číst</button>
        </div>`).join('');
      h += `<div style="font-size:.66rem;color:var(--text-3);margin-top:.4em">Štítky ukazují celý pokrytý podprostor každého podání — jedno podání může obsloužit víc políček mapy.</div>`;
    }
    h += '</div>';
    box.innerHTML = h;
  ...
  _renderSavedList() {
    ...
    if (!saved.length) {
      return `<div style="text-align:center;padding:2em;color:var(--text-3);background:var(--surface);border:1px solid var(--border);border-radius:var(--radius)">
        ...
        <p style="font-size:.78rem;color:var(--text-3);margin-top:.2em">Klepni na ikonu záložky u kterékoliv části a ulož si ji na později.</p>
      </div>`;
    }

    let html = `<div style="font-size:.82rem;color:var(--text-2);margin-bottom:.8em">Uloženo: ${saved.length} — klepni a čti, křížkem odebereš</div>`;
    html += '<div class="map-blocks">';
  ...
  _renderNotesList() {
    ...
    if (!allNotes.length) {
      return `<div style="text-align:center;padding:2em;color:var(--text-3);background:var(--surface);border:1px solid var(--border);border-radius:var(--radius)">
        ...
        <p style="font-size:.78rem;color:var(--text-3);margin-top:.2em">Označ text a klepni na „Zvýraznit + poznámka“, nebo klepni na ikonu poznámky u kterékoliv části.</p>
      </div>`;
    }

    let html = `<div style="font-size:.82rem;color:var(--text-2);margin-bottom:.8em">Poznámek: ${allNotes.length}</div>`;
    html += '<div class="map-blocks">';
    allNotes.forEach(n => {
  ...
  _renderFacetProfile() {
    ...
    const FACET_WORDS = {
      lens: { generic: 'příklady odevšad', ecommerce: 'příklady z nakupování', media: 'příklady z hudby a videí', 'social-feeds': 'příklady ze sociálních sítí', education: 'příklady ze školy a učení' },
      visuality: { 'text-first': 'textová vysvětlení', balanced: 'mix textu a diagramů', 'visual-first': 'vizuální vysvětlení' },
      depth: { intro: 'jemné úvody', standard: 'standardní hloubka', technical: 'technická hloubka', research: 'výzkumná hloubka' },
      formalism: { none: 'bez vzorců', light: 'pár vzorců', full: 'plná matematika' },
    };
    let h = '';
    ...
    // (arrow-key chapter hops made readers lose their place — this is the undo)
    {
      ...
      if (seen.size) {
        ...
        h += '<div class="profile-section"><h3>🕘 Naposledy zobrazené</h3><div class="history-list">';
        for (const [id, { meta, ts }] of seen) {
          const when = ts ? this._timeAgo(ts) : '';
          const mark = edited.has(id) || this._overrides?.[id] ? ' <span class="hist-edit" title="tady máš úpravy">✏️</span>' : '';
    ...
    h += '<p style="font-size:.78rem;color:var(--text-2);margin-bottom:.6em">Model preferencí, který se o tobě kniha naučila — přesně takový si staví každý doporučovací systém. Kdykoliv ho můžeš opravit; tvoje opravy mají vždy přednost.</p>';

    // Visual affinity bars (top values per primary facet)
    const FACET_LABELS = { lens: '🌐 Svět příkladů', depth: '📏 Hloubka', visuality: '🖼 Vizuální styl' };
    let hasBars = false;
    ...
    if (!hasBars) {
      h += '<p style="font-size:.85rem;margin-bottom:.7em">Kniha tě teprve poznává — přečti pár částí, klepni na 👍 nebo si dolaď libovolné podání a tohle se doplní.</p>';
    }
    ...
    if (priv.length || this.user.readerMode === 'open') {
      ...
      const shareRows = sharedList.slice(0, 5).map(b =>
        `<div style="font-size:.72rem;padding:.1em 0">⚡ ${this.escHtml(b.meta.title || b.meta.id)}
          <button class="steer-chip" style="font-size:.62rem;padding:.05em .4em" onclick="app.shareThing('${this.escHtml(b.meta.title || 'Moje podání')}', 'Tohle podání jsem napsal/a v živé knize „Jak funguje internet?“:', location.origin + '/#${b.meta.id}')">🔗 sdílet</button></div>`).join('');
      h += (shareRows ? `<div style="margin:.3em 0">${shareRows}</div>` : '');
      h += `<div class="dna-contrib">🌱 <b>Tvoje příspěvky do živé knihy:</b>
        ${generated ? `vygenerovaná podání: ${generated} · ` : ''}${remixes ? `remixy: ${remixes} · ` : ''}${shared ? `<b>sdíleno se čtenáři: ${shared}</b> · ` : ''}${!priv.length ? 'zatím nic — označ libovolnou pasáž a klepni na ✨, nebo najdi mezeru na ' : 'podívej se na '}<a href="#" onclick="app.switchView('map');app.setMapMode('coverage');return false">🌱 mapě</a></div>`;
    }
    ...
    const PREF_DIMS = [
      { facet: 'lang', label: '🌍 Jazyk', words: { en: 'anglicky', cs: 'česky' } },
      { facet: 'lengthBand', label: '⏱ Délka', words: { tldr: 'krátce a jasně', standard: 'standardní', deep: 'dlouhé čtení' } },
      { facet: 'visuality', label: '🖼 Forma', words: FACET_WORDS.visuality },
      { facet: 'depth', label: '📏 Hloubka', words: FACET_WORDS.depth },
      { facet: 'lens', label: '🌐 Svět', words: FACET_WORDS.lens },
      { facet: 'carriers', label: '🧩 Prvky', words: { prose: 'text', table: 'tabulky', diagram: 'diagramy', image: 'obrázky', animation: 'animace', formula: 'vzorce', code: 'kód' } },
    ];
    h += '<div style="margin-top:.6em;padding-top:.5em;border-top:1px solid var(--border)"><b style="font-size:.8rem">🎛 Předvolby formátu</b><p style="font-size:.68rem;color:var(--text-3);margin:.15em 0 .4em">Jak ti má kniha věci vyprávět? Tvoje výslovné volby mají vždy přednost před naučeným modelem.</p>';
    ...
    if (this._f('community') || this._f('generation')) {
      h += `<div style="margin-top:.8em;padding-top:.6em;border-top:1px solid var(--border)">
        <label class="intro-toggle" style="display:flex;gap:.5em;align-items:flex-start">
          ...
          <div><b>Otevřený režim</b> — živá kniha<br><span class="toggle-desc" style="font-size:.7rem">Zobrazovat i obsah generovaný na přání a varianty sdílené ostatními čtenáři (jasně označené, zatím neověřené redakcí). Vypnuto = jen ověřený obsah.</span></div>
        </label></div>`;
    }
    h += '</div>';
    return h;
  ...
  correctFacetPref(facet, value) {
    ...
    this.showXPToast('Předvolba uložena', 'achievement');
  ...
  renderProfile() {
    ...
    if (auth) {
      ...
      h += `<div class="auth-card" style="display:flex;align-items:center;gap:.8em">
        ...
        <div style="flex:1">
          <div style="font-size:.82rem;font-weight:600">Nejsi přihlášen/a</div>
          <div style="font-size:.72rem;color:var(--text-3)">Tvůj postup se ukládá jen v tomhle zařízení.</div>
        </div>
    ...
    if (!this._f('gamification')) {
      h += `<div class="profile-section"><h3>Postup čtení</h3><p style="font-size:.85rem">Přečteno ${p.progress.read} z ${p.progress.total} částí (${p.progress.pct} %)</p><p style="font-size:.8rem;color:var(--text-2)">Celkem ${p.readingTimeMin} min čtení</p></div>`;
    } else {
    const xpInLevel = u.getXPInCurrentLevel();
    const xpNeeded = u.getXPForNextLevel();
    const xpPct = Math.min(100, Math.round((xpInLevel / xpNeeded) * 100));
    h += `<div class="gami-hero">
      <div class="gami-level">Úr. ${u.level}</div>
      <div class="gami-info">
    ...
    if (this._f('spaceRepetition')) {
      ...
      h += '<div class="profile-section"><h3>\u{1F9E0} Paměťové kartičky</h3>';
      ...
      if (totalRecall > 0) {
        ...
        if (medC) h += `<span style="color:var(--warn)">${medC} v učení</span>`;
        if (easyC) h += `<span style="color:var(--product)">${easyC} s jistotou</span>`;
        h += `</div>`;
        ...
        const upcoming = Object.entries(u.recall)
          ...
          .slice(0, 3);
        if (upcoming.length) {
          h += `<div style="font-size:.68rem;color:var(--text-3);margin-bottom:.5em">Nejbližší opakování: ${upcoming.map(([id, c]) => {
      ...
      h += `<span>${totalRecall} sledovaných</span>`;
      ...
      h += `<button class="btn-primary" style="flex:1;font-size:.78rem;padding:.4em" onclick="app.switchView('quiz')">\u{1F9E0} Záložka Kvíz</button>`;
      if (dueCount > 0) h += `<button class="recall-reveal" style="flex:1" onclick="app.startPractice(true)">Zopakovat (${dueCount})</button>`;
      h += `</div>`;
      h += `</div>`;
    }
    ...
    const allBadges = [
      { id: 'first_read', icon: '👣', name: 'První krůčky' }, { id: 'reader_5', icon: '📚', name: 'Knihomol' },
      { id: 'reader_15', icon: '⚡', name: 'Rychločtenář' }, { id: 'reader_30', icon: '🤖', name: 'Stroj na znalosti' },
      { id: 'first_like', icon: '❤️', name: 'Palec nahoru' }, { id: 'like_10', icon: '🌟', name: 'Super fanoušek' },
      { id: 'first_note', icon: '📝', name: 'Zapisovatel' }, { id: 'voice_all', icon: '🎭', name: 'Trojitá hrozba' },
      ...
      { id: 'xp_200', icon: '💎', name: 'Lovec XP' }, { id: 'deep_diver', icon: '🤿', name: 'Hlubinný potápěč' },
      { id: 'recall_5', icon: '🧠', name: 'Paměťový profík' },
      { id: 'certified', icon: '🎓', name: 'S certifikátem!' },
    ];
    ...
    h += '<span style="font-weight:600;color:var(--accent)">+5 XP</span><span>Prozkoumej prohlubující kartu</span>';
    h += '<span style="font-weight:600;color:var(--accent)">+5 XP</span><span>Napiš poznámku</span>';
    h += '<span style="font-weight:600;color:var(--accent)">+3 XP</span><span>Dej lajk části</span>';
    h += '<span style="font-weight:600;color:var(--accent)">+2 XP</span><span>Ulož na později</span>';
    h += '<span style="font-weight:600;color:#F59E0B">+2-8 XP</span><span>Opakovací kartička (podle toho, jak dobře si pamatuješ)</span>';
    h += '</div></div>';
    ...
    h += `<div class="profile-section"><h3>🛠 Redaktorská dráha</h3>
      <div style="font-size:.8rem;margin-bottom:.4em">${
        et.tier === 'editor'
          ? `<span class="editor-badge">🛠 REDAKTOR</span> ${et.invited ? 'Pozvání do redakčního týmu — vítej.' : 'Redaktorství sis zasloužil/a — tvoje příspěvky utvářejí knihu.'} <a href="admin.html" style="color:var(--accent)">Otevřít redakční konzoli</a> · <a href="HUMANS.md" style="color:var(--accent)">povinnosti</a>`
          : et.tier === 'contributor'
            ? `<b>Přispěvatel.</b> Jen tak dál — redaktorství se získává, nedostává:`
            : `<b>Čtenář.</b> Redaktorství se získává, nedostává — hlas chyby, sdílej podání:`}</div>
      ${etBar('🚩 nahlášené chyby', et.flags, 5)}
      ${etBar('⚡ sdílená podání', et.shared, 5)}
      ${etBar('📖 přijato do knihy', et.adopted, 1)}
      <p style="font-size:.68rem;color:var(--text-3);margin-top:.3em">${et.invited ? 'Redaktorem ses stal/a na pozvání — ukazatele navíc ukazují tvoje vlastní příspěvky.' : 'Povýšení: <b>1 podání přijaté</b> do knihy (redakce ho prověřila a začlenila) — nebo <b>5 sdílených + 5 nahlášených</b>. Přijetí se pozná automaticky, když se tvoje sdílené podání objeví v knize.'}</p>
      ${et.adopted >= 1 ? `<button class="steer-chip" style="border-color:var(--accent);color:var(--accent)" onclick="app.shareThing('Moje podání se dostalo do knihy', 'Moje vysvětlení bylo přijato do živé knihy „Jak funguje internet?“ 📖', location.origin + '/#' + (Object.values(app.privateBlocks).find(b => (b.meta.sharedAt || b.meta.state === 'community') && app.findBlock(b.meta.id))?.meta.id || ''))">📣 Pochlub se přijetím</button>` : ''}
    </div>

    <div class="profile-section"><h3>📣 Pozvi kamarády</h3>
      <p style="font-size:.78rem;color:var(--text-2)">S více čtenáři je kniha lepší — každý hlas, doladění i přání ovlivňuje, co se napíše. Pozvaní kamarádi začínají s <b>+25 XP</b>.</p>
      <div style="display:flex;gap:.4em;flex-wrap:wrap;align-items:center">
        ...
        <button class="steer-chip" onclick="navigator.clipboard.writeText(document.getElementById('inviteLink').textContent).then(()=>app.showXPToast('🔗 Odkaz s pozvánkou zkopírován','achievement'))">kopírovat</button>
        <button class="steer-chip" style="border-color:var(--accent);color:var(--accent)" onclick="app.shareThing('Jak funguje internet?','Čtu živou knihu, která vysvětluje, jak funguje internet — a přizpůsobuje se ti. Přidej se:', document.getElementById('inviteLink').textContent)">sdílet</button>
      </div>
    </div>`;
    ...
    if (achievedIdx >= 0) {
      ...
      h += `<div class="cert-ready">
        <p style="font-size:.85rem;margin-bottom:.6em">${t.icon} Získal/a jsi certifikát <strong>${t.title}</strong> — ${t.subtitle}.</p>
        <button class="cert-btn" onclick="app.showCertificateModal()">Získat certifikát ${t.title}</button>
  ...
  async register() {
    ...
    if (!email || !password) { errEl.textContent = 'E-mail a heslo jsou povinné.'; return; }
    if (password.length < 4) { errEl.textContent = 'Heslo musí mít aspoň 4 znaky.'; return; }
    errEl.textContent = 'Vytvářím účet…';
    ...
    this.showXPToast('Účet vytvořen! Tvůj postup se teď synchronizuje.', 'info');
  ...
  getMissions() {
    ...
    return [
      {
        id: 'hodina1', title: 'Vstupenka: Jak vypadá internet', icon: '\u{1F5FA}',
        difficulty: 'Za\u010d\u00e1te\u010dn\u00edk',
        story: 'P\u0159\u00ed\u0161t\u00ed hodinu za\u010d\u00edn\u00e1te mluvit o tom, jak internet doopravdy vypad\u00e1. Projdi dom\u00e1c\u00ed p\u0159\u00edpravu a z\u00edskej vstupenku.',
        goal: 'Um\u011bt vysv\u011btlit, z \u010deho se internet skl\u00e1d\u00e1: servery, kabely, routery',
        reward: { title: 'Vstupenka na 2. hodinu', xp: 20 },
        core: ['ch1-servery', 'ch1-kabely', 'ch1-routery', 'ch1-nema-reditele', 'ch1-shrnuti'],
        intros: [
          'Kde vlastn\u011b bydl\u00ed vide\u00e1 a zpr\u00e1vy? Spoiler: ne v obl\u00e1\u010dku.',
          'Mezi kontinenty data neputuj\u00ed vzduchem. Le\u017e\u00ed na dn\u011b oce\u00e1nu.',
          'Kdo uk\u00e1\u017ee tv\u00e9 zpr\u00e1v\u011b cestu? Chytr\u00e9 k\u0159i\u017eovatky \u2014 routery.',
          'Internet nem\u00e1 \u0159editele. A pr\u00e1v\u011b proto ho nejde vypnout.',
          'Zopakuj si celou skl\u00e1da\u010dku, ne\u017e vyraz\u00ed\u0161 na boss ot\u00e1zku.'
        ],
        boss: { q: 'Kamar\u00e1d ti tvrd\u00ed: \u201eInternet je jeden ob\u0159\u00ed po\u010d\u00edta\u010d n\u011bkde v Americe a data k n\u00e1m l\u00e9taj\u00ed p\u0159es satelity. Kdy\u017e ho n\u011bkdo vypne, internet skon\u010d\u00ed.\u201c Vysv\u011btli mu, co v\u0161echno m\u00e1 \u0161patn\u011b.', hints: ['servery', 'datov\u00e1 centra', 'podmo\u0159sk\u00e9 kabely', 'routery', 'decentralizace'] },
        branches: {
          explorer: { label: 'Prozkoumat mapy', blocks: ['ch1-mapa-kabelu'] },
          creator: { label: 'Nakreslit s\u00ed\u0165', blocks: ['ch1-nakresli-sit'] },
          thinker: { label: 'Pochopit pakety', blocks: ['ch1-klient-server', 'ch1-pakety', 'ch1-obsah-paketu'] }
        }
      },
      {
        id: 'hodina2', title: 'Vstupenka: Jak se p\u0159ipojujeme', icon: '\u{1F4F6}',
        difficulty: 'Za\u010d\u00e1te\u010dn\u00edk',
        story: 'Wi-Fi doma, mobil venku, kabel ve \u0161kole \u2014 p\u0159\u00ed\u0161t\u011b se \u0159e\u0161\u00ed, jak se za\u0159\u00edzen\u00ed dostanou do s\u00edt\u011b. P\u0159iprav se a z\u00edskej vstupenku.',
        goal: 'Zn\u00e1t typy p\u0159ipojen\u00ed a um\u011bt vybrat to prav\u00e9 pro danou situaci',
        reward: { title: 'Vstupenka na 3. hodinu', xp: 20 },
        core: ['ch2-wifi-router', 'ch2-kabely', 'ch2-mobilni-data', 'ch2-hotspot-satelity', 'ch2-shrnuti'],
        intros: [
          'Za\u010dneme doma: co v\u0161echno d\u011bl\u00e1 ta blikaj\u00edc\u00ed krabi\u010dka \u2014 router?',
          'Po dr\u00e1t\u011b je to jistota. UTP, optika a kabely pod oce\u00e1nem.',
          'Venku bez Wi-Fi? Zachr\u00e1n\u00ed t\u011b vys\u00edla\u010de BTS a SIM karta.',
          'Hotspot z mobilu a satelity \u2014 z\u00e1chrana tam, kam kabel nevede.',
          'Shr\u0148 si v\u0161echny cesty do internetu, ne\u017e vyraz\u00ed\u0161 na bosse.'
        ],
        boss: { q: 'Kamar\u00e1d tvrd\u00ed: \u201eZpr\u00e1va do Ameriky let\u00ed p\u0159es satelit, proto je internet ob\u010das pomal\u00fd.\u201c Vysv\u011btli, kudy zpr\u00e1va doopravdy putuje a pro\u010d je to rychlej\u0161\u00ed ne\u017e satelit.', hints: ['podmo\u0159sk\u00e9 kabely', 'optika', '35 000 km', 'p\u00e1te\u0159n\u00ed s\u00ed\u0165'] },
        branches: {
          explorer: { label: 'Opravy kabel\u016f', blocks: ['ch2-opravy-kabelu'] },
          creator: { label: 'Zm\u011b\u0159it rychlost', blocks: ['ch2-speedtest-experiment'] },
          thinker: { label: 'Rychlost a optika', blocks: ['ch2-prenosova-rychlost', 'ch2-utp-vs-optika', 'ch2-satelity-starlink'] }
        }
      },
      {
        id: 'hodina3', title: 'Vstupenka: Co o n\u00e1s v\u00ed internet', icon: '\u{1F463}',
        difficulty: 'M\u00edrn\u011b pokro\u010dil\u00fd',
        story: 'Ka\u017ed\u00fd klik po tob\u011b n\u011bco zanech\u00e1. P\u0159\u00ed\u0161t\u00ed hodina je o digit\u00e1ln\u00ed stop\u011b \u2014 p\u0159ijdi p\u0159ipraven\u00fd a se vstupenkou.',
        goal: 'Pochopit pakety, cookies a digit\u00e1ln\u00ed stopu \u2014 a um\u011bt ji m\u00edt pod kontrolou',
        reward: { title: 'Vstupenka na 4. hodinu', xp: 20 },
        core: ['ch3-dino', 'ch3-pakety', 'ch3-kdo-plati', 'ch3-cookies', 'ch3-stopa'],
        intros: [
          'Jak p\u0159ev\u00e9zt dinosaura? Rozlo\u017een\u00e9ho, t\u0159emi n\u00e1klad\u00e1ky. P\u0159esn\u011b tak cestuj\u00ed data.',
          'Ka\u017ed\u00fd paket nese adresu. Bez IP adresy by se nikdy nedo\u0161el dom\u016f.',
          'Internet nen\u00ed zadarmo. Kdo ho tedy plat\u00ed \u2014 a \u010d\u00edm plat\u00ed\u0161 ty?',
          'Cookie je \u0161atnov\u00fd l\u00edstek: mal\u00fd pap\u00edrek, kter\u00fd si t\u011b pamatuje.',
          'Poskl\u00e1dej si, co v\u0161echno tvo\u0159\u00ed tvou stopu \u2014 a jak ji \u010d\u00edst.'
        ],
        boss: { q: 'Kamar\u00e1dka \u0159\u00edk\u00e1: \u201eInternet je zadarmo a nikdo nev\u00ed, co na n\u011bm d\u011bl\u00e1m.\u201c Ob\u011b \u010d\u00e1sti t\u00e9 v\u011bty jsou \u0161patn\u011b \u2014 vysv\u011btli pro\u010d.', hints: ['reklamy', 'ISP', 'digit\u00e1ln\u00ed stopa', 'historie vyhled\u00e1v\u00e1n\u00ed'] },
        branches: {
          explorer: { label: 'Kolik reklam vid\u00ed\u0161?', blocks: ['ch3-reklamy-sidebar'] },
          creator: { label: 'Detektiv k\u0159e\u010dek', blocks: ['ch3-detektiv-krecek'] },
          thinker: { label: 'Cookies a Google', blocks: ['ch3-cookies-d-think', 'ch3-google-d-think'] }
        }
      },
      {
        id: 'hodina4', title: 'Velk\u00e9 opakov\u00e1n\u00ed: cel\u00fd blok', icon: '\u{1F3C6}',
        difficulty: 'M\u00edrn\u011b pokro\u010dil\u00fd',
        story: '\u010ctvrt\u00e1 hodina je fin\u00e1le: obhajoby telling\u016f a boss kv\u00edz. Zopakuj si cel\u00fd blok a z\u00edskej certifik\u00e1t.',
        goal: 'Propojit v\u0161echno: s\u00ed\u0165, p\u0159ipojen\u00ed i digit\u00e1ln\u00ed stopu',
        reward: { title: 'Certifik\u00e1t bloku', xp: 40 },
        core: ['ch4-jak-vypada-internet', 'ch4-jak-se-pripojujeme', 'ch4-digitalni-stopa'],
        intros: [
          'Servery, kabely, routery \u2014 slo\u017e si znovu obr\u00e1zek cel\u00e9 s\u00edt\u011b.',
          'Wi-Fi, dr\u00e1t, mobil, satelit: kter\u00e9 p\u0159ipojen\u00ed kdy d\u00e1v\u00e1 smysl?',
          'A nakonec stopa: co po tob\u011b z\u016fst\u00e1v\u00e1 a kdo s t\u00edm pracuje?'
        ],
        boss: { q: 'Popi\u0161 celou cestu zpr\u00e1vy od tv\u00e9ho telefonu ke kamar\u00e1dovi na druh\u00e9m konci sv\u011bta \u2014 a co po n\u00ed z\u016fstane. Pou\u017eij co nejv\u00edc pojm\u016f z bloku.', hints: ['paket', 'router', 'Wi-Fi', 'IP adresa', 'podmo\u0159sk\u00fd kabel', 'server', 'digit\u00e1ln\u00ed stopa'] },
        branches: {
          explorer: { label: 'Zahr\u00e1t si Bingo', blocks: ['ch4-bingo'] },
          creator: { label: 'M\u016fj telling', blocks: ['ch4-muj-telling'] },
          thinker: { label: 'T\u011b\u017e\u0161\u00ed pojmy', blocks: ['ch4-tezsi-pojmy'] }
        }
      }
    ];
  }
  ...
  _personalMissionCard() {
    ...
    if (done && !pm.rewarded) {
      ...
      setTimeout(() => this.showXPToast('🎯 Osobní mise splněna! +50 XP', 'achievement'), 800);
    }
    ...
    return `<div class="mission-card mission-active" style="border:2px solid var(--accent)">
      ...
      <div class="mission-info">
        <div class="mission-title-row"><span class="mission-title">Tvoje mise: ${this.escHtml(pm.goal)}</span>
          <span class="mission-diff" style="background:var(--accent)">OSOBNÍ</span></div>
        <div style="font-size:.72rem;color:var(--text-3);margin:.2em 0">Sestaveno z ${pm.ids.length} konceptů odpovídajících tvému cíli — přečti je všechny a misi splníš. <a href="#" onclick="event.preventDefault();app.editPersonalMission()" style="color:var(--accent)">změnit cíl</a></div>
        ${steps}
        <div style="font-size:.72rem;color:var(--text-2);margin-top:.3em">${read}/${pm.ids.length} přečteno${done ? ' · splněno 🎉' : ''}
          ${done ? `<button class="steer-chip" style="margin-left:.5em;border-color:var(--accent);color:var(--accent)" onclick="app.shareThing('Mise splněna', 'Právě jsem dokončil/a osobní misi „${this.escHtml(pm.goal)}“ v živé knize Jak funguje internet? 🎯', location.origin)">📣 sdílet</button>` : ''}</div>
      </div>
    </div>`;
  }
  ...
  editPersonalMission() {
    ...
    const text = window.prompt('🎯 Co se chceš naučit? Kniha sestaví misi z odpovídajících konceptů.', cur);
    if (text === null) return;
    ...
    if (!m) this.showXPToast('Žádné odpovídající koncepty — zkus to říct jinak', 'achievement');
    this.renderMissions();
  }
  ...
  renderMissions() {
    ...
    html += '<div class="missions-head"><h2>Mise</h2><p>Vyber si své dobrodružství. Každá mise vypráví příběh a naučí tě něco nového.</p></div>';
    html += this._personalMissionCard() || `<div style="font-size:.75rem;color:var(--text-3);margin:.2em 0 .6em">🎯 <a href="#" onclick="event.preventDefault();app.editPersonalMission()" style="color:var(--accent)">Řekni knize, co se chceš naučit</a> — sestaví ti osobní misi z odpovídajících konceptů.</div>`;
    ...
    missions.forEach(m => {
      ...
      const diffColors = { 'Beginner': '#10B981', 'Intermediate': '#F59E0B', 'Advanced': '#EF4444', 'Capstone': '#8B5CF6', 'Začátečník': '#10B981', 'Mírně pokročilý': '#F59E0B', 'Pokročilý': '#EF4444' };
      ...
      html += `<div class="mission-card ${isComplete ? 'mission-complete' : ''} ${isLocked ? 'mission-locked' : ''} ${isActive ? 'mission-active' : ''}" onclick="${isLocked ? '' : `app.showMission('${m.id}')`}">
        ...
        <div class="mission-info">
          ...
          <div class="mission-story">${isLocked ? `Odemkneš dokončením ${m.prerequisite} misí` : m.story.substring(0, 80) + '…'}</div>
          ...
          <div class="mission-meta">
            ...
            ${isComplete ? '<span class="mission-complete-badge">\u2713 Splněno</span>' : `<span>${prog.read}/${m.core.length}</span>`}
          </div>
        </div>
      </div>`;
    });
  ...
  showMission(missionId) {
    ...
    html += `<div class="mission-detail-header">
      ...
      <button class="act-btn share-btn" onclick="app.shareMission('${m.id}')" title="Sdílet tuhle misi" style="margin-left:auto;font-size:1rem">&#128279;</button>
    </div>`;
    ...
    html += `<div class="mission-detail-progress">
      ...
      <div style="text-align:center;font-size:.75rem;color:var(--text-3);margin-top:.3em">${coreRead}/${m.core.length} hlavních kroků${branch ? ` + bonus ${m.branches[branch].blocks.filter(id => this.user.readBlocks.has(id)).length}/${m.branches[branch].blocks.length}` : ''}</div>
    </div>`;

    // Step list — core blocks
    html += '<div class="mission-steps">';
    html += '<div class="mission-steps-label">TVOJE CESTA</div>';
    m.core.forEach((id, i) => {
      const block = this.findBlock(id);
      const isRead = this.user.readBlocks.has(id);
    ...
    if (!branch) {
      html += `<div class="mission-branch-point">
        <div class="branch-label">Vyber si cestu</div>
        <div class="branch-desc">Tady se příběh větví. Vyber si svůj styl — kniha se ti přizpůsobí!</div>
      ...
      Object.entries(m.branches).forEach(([voice, b]) => {
        ...
        html += `<button class="branch-option ${voice}" onclick="app.pickBranch('${m.id}','${voice}')">
          ...
          <span class="branch-count">+${b.blocks.length} částí</span>
        </button>`;
      });
      html += '</div></div>';
    } else {
      ...
      html += `<div class="mission-branch-chosen">
        <div class="branch-label">${vc.icon || ''} cesta ${vc.label || branch} <button class="btn-ghost" style="font-size:.7rem" onclick="app.pickBranch('${m.id}',null)">změnit</button></div>
      </div>`;
      b.blocks.forEach((id, i) => {
        ...
        html += `<div class="mission-step ${isRead ? 'step-done' : ''} step-branch" onclick="app.openBlock('${id}')">
          ...
          <div class="step-content">
            <div class="step-title">${title}</div>
            <div class="step-meta">cesta ${vc.label} &middot; ${block?.meta?.readingTime || 3} min</div>
          </div>
        </div>`;
      });
    }
    ...
    if (isComplete) {
      html += `<div class="mission-reward-earned">
        ...
        <div class="reward-text">Mise splněna! Získáváš: <b>${m.reward.title}</b> +${m.reward.xp} XP</div>
      </div>`;
    } else if (coreComplete) {
      html += `<div class="mission-complete-action">
        <button class="mission-complete-btn" onclick="app._startBossQuiz('${m.id}')">Utkej se s bossem &rarr;</button>
      </div>`;
    }
  ...
  _renderWizardStep() {
    ...
    <div class="wizard-progress-label">${m.icon} ${m.title} &middot; Krok ${step + 1} z ${blocks.length}</div>`;
    ...
    html += `<div class="wizard-block-card" onclick="app._pendingMissionIntro='${(intro||'').replace(/'/g,"\\'")}';app.openBlock('${blockId}')">
      ...
      <button class="wizard-read-btn">${isRead ? '\u2713 Přečíst znovu' : 'Přečíst tuhle část'} &rarr;</button>
    </div>`;
    ...
    html += `<div class="wizard-nav">
      ...
      ${isRead ? `<button class="wizard-nav-btn wizard-nav-next" onclick="app._wizardStep=${step + 1};app._renderWizardStep()">Další &rarr;</button>` : `<button class="wizard-nav-btn" disabled style="opacity:.4">Pokračuj přečtením</button>`}
    </div>`;
  ...
  _renderBossQuiz() {
    ...
    <div class="wizard-progress-label">${m.icon} ${m.title} &middot; Závěrečná výzva!</div>`;

    html += `<div class="boss-card">
      <div class="boss-icon">\u{1F409}</div>
      <h3 class="boss-title">Boss: Dokaž, co ses naučil/a!</h3>
      <div class="boss-question">${m.boss.q}</div>
      <textarea class="boss-answer" id="bossAnswer" placeholder="Sem napiš svou odpověď…" rows="4"></textarea>
      <div class="boss-hint" id="bossHint" style="display:none"></div>
      <div class="boss-actions">
        <button class="boss-submit" id="bossSubmit" onclick="app._checkBossAnswer('${m.id}')">Zkontrolovat odpověď</button>
    ...
    this._probeBoss().then(on => {
      ...
      if (note) note.textContent = on ? '🤖 hodnotí AI zkoušející' : '';
    });
  }
  ...
  async _checkBossAnswer(missionId) {
    ...
    if (rawAnswer.length < 20) {
      ...
      hintEl.innerHTML = 'Napiš toho víc! Zkus to vysvětlit aspoň v pár větách.';
      return;
    }
    ...
    if (await this._probeBoss()) {
      ...
      if (btn) { btn.disabled = true; btn.textContent = 'Zkoušející čte… 🧐'; }
      hintEl.style.display = 'block';
      hintEl.className = 'boss-hint';
      hintEl.innerHTML = '<span class="gen-spinner">Drak zvažuje tvou odpověď…</span>';
      try {
        const res = await fetch('/api/boss', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        const data = await res.json();
        if (!res.ok || !data.ok) throw new Error(data.error || 'hodnocení selhalo');
        const g = data.grade;
        ...
        if (g.verdict === 'pass') {
          ...
          hintEl.innerHTML = `${bar}${this.escHtml(g.feedback)}
            ${g.missing?.length ? `<div style="margin-top:.4em">Ještě chybí: ${g.missing.map(x => `<b>${this.escHtml(x)}</b>`).join(' · ')}</div>` : ''}
            ${g.followUp ? `<div style="margin-top:.4em">🐉 <i>${this.escHtml(g.followUp)}</i></div>` : ''}
            <div style="margin-top:.5em">
              <button class="wizard-nav-btn" onclick="document.getElementById('bossAnswer').focus()">Vylepšit odpověď</button>
              <button class="wizard-nav-btn" onclick="app._wizardStep=0;app._renderWizardStep()">Projít kroky znovu</button>
            </div>`;
        }
        if (btn) { btn.disabled = false; btn.textContent = 'Zkontrolovat odpověď'; }
        return;
      } catch (e) {
        if (btn) { btn.disabled = false; btn.textContent = 'Zkontrolovat odpověď'; }
  ...
  _checkBossAnswerLocal(missionId, answer, hintEl) {
    ...
    if (score >= 0.5 || answer.length > 80) {
      ...
      hintEl.style.display = 'block';
      hintEl.className = 'boss-hint boss-retry';
      hintEl.innerHTML = `Dobrý začátek! Zkus ale zmínit i: <b>${missing.join('</b> a <b>')}</b>. Klidně se vrať a přečti si to znovu!`;
      hintEl.innerHTML += `<br><button class="wizard-nav-btn" style="margin-top:.4em" onclick="app._wizardStep=0;app._renderWizardStep()">Projít kroky znovu</button>`;
    }
  }
  ...
  generateChatResponse(query, targetEl) {
    ...
    if (result.canEscalate) {
      html += `<br><br><button class="tutor-escalate-btn" onclick="app.escalateToAuthor()">Napsat autorům</button>`;
    }
    return html;
  }
  ...
  escalateToAuthor() {
    ...
    const msgHtml = '<div class="chat-msg system">Tvoje otázka byla odeslána autorům knihy. Každou zprávu si přečtou! Mezitím zkus prozkoumat související části knihy.</div>';
  ...
  askAboutBlock(blockId) {
    ...
    if (chatEl) {
      chatEl.innerHTML = `<div class="chat-msg bot">V knize je o tématu <b>${block.meta.title}</b> spousta informací. Co by tě zajímalo? Umím vysvětlovat, najít související části nebo jít víc do hloubky!</div>`;
      if (questions.length) {
  ...
  async onSearch(query) {
    ...
    if (!query || query.length < 2) { el.innerHTML = '<div class="search-empty">Piš a prohledáš celý obsah…</div>'; return; }
    ...
    const renderResults = (results) => {
      if (!results.length) { el.innerHTML = '<div class="search-empty">Nic jsme nenašli.</div>'; return; }
  ...
  answerQ(el, voice, qId) {
    ...
    if (recs.length) {
      html += `<div class="q-recs-title">Co číst dál${vc.label ? ' (cesta ' + vc.label + ')' : ''}:</div>`;
  ...
  _generateAnswerFeedback(qId, letter, voice, text) {
    ...
    const voiceFeedback = {
      explorer: 'Skvělá volba! Jako Průzkumník si užiješ praktické ukázky a vizuální vysvětlení. Pojďme se podívat, jak to funguje pod kapotou!',
      creator: 'Super — vybral/a sis cestu Tvůrce! Připrav se na projekty, experimenty a stavění skutečných věcí. Učení děláním je nejlepší!',
      thinker: 'Pěkné — jsi Myslitel! Rád/a chápeš, PROČ věci fungují. Hlubší vysvětlení, která tě čekají, jsou přesně pro tebe.',
      universal: 'Skvělá volba! Čeká tě mix všeho — zkoumání, tvoření i přemýšlení. Jedeme dál!'
    };
  ...
  toggleLike(blockId) {
    ...
    if (btn) {
      ...
      btn.innerHTML = newRating >= 0.7 ? '&#10084;&#65039; <span>Líbí se</span>' : '&#9825; <span>Líbí?</span>';
    }
  ...
  remixSelection() {
    ...
    const blockId = anchor?.id?.replace('b-', '');
    // The paragraph the cursor sits in — the place a new passage would go.
  ...
  async _openRemixForm(blockId, quote, opts = {}) {
    ...
    if (this.user.readerMode !== 'open') {
      box.innerHTML = `<b>✨ Remix</b><div style="font-size:.75rem;margin-top:.3em">Remixem si vytvoříš vlastní soukromou verzi této části. Nejdřív si zapni <b>otevřený režim</b> v <a href="#" onclick="app.switchView('profile');return false">Profilu</a>.</div>`;
    } else {
      ...
      const scopeLabel = quote ? 'tuhle pasáž' : 'tuhle část';
      const whereHint = insert
        ? (opts.anchorText
            ? `<div style="font-size:.68rem;color:var(--text-3);margin:.2em 0">Vloží se sem: za „${this.escHtml(opts.anchorText.slice(0, 70))}${opts.anchorText.length > 70 ? '…' : ''}“</div>`
            : '<div style="font-size:.68rem;color:var(--text-3);margin:.2em 0">Vloží se na konec této části.</div>')
        : '';
      box.innerHTML = `
        <b>${insert ? '➕ Přidat sem' : `✏️ Vylepšit ${scopeLabel}`}</b>
        ${whereHint}
        <div style="font-size:.68rem;color:var(--text-3);margin:.2em 0 .1em">${insert ? 'Napiš vlastní text…' : 'Uprav text přímo…'}</div>
        <textarea id="edit-src-${blockId}" rows="${insert ? 3 : Math.min(12, Math.max(3, slice.split('\n').length + 1))}"
          ${insert ? 'placeholder="Text, který se na tomhle místě objeví…"' : ''}
        ...
        ${canGen ? `<div style="font-size:.68rem;color:var(--text-3);margin:.3em 0 .1em">…nebo popiš ${insert ? 'co sem přidat' : 'změnu'} a nech to napsat AI:</div>
  ...
  submitManualEdit(blockId) {
    ...
    if (ctx.insert) {
      if (!edited.trim()) { this.showXPToast('Napiš text, který se má vložit', 'xp'); return; }
      marked = this._markRegion(edited);
      oldMid = ''; newMid = edited.trim();
      newBody = this._spliceIn(base, span[0], marked);
    } else {
      if (edited.trim() === original.trim()) { this.showXPToast('Zatím žádná změna — nejdřív text uprav', 'xp'); return; }
  ...
  _attachParaTools(root) {
    ...
    scope.querySelectorAll('.block-article').forEach(article => {
      ...
      paras.forEach(el => {
        ...
        tools.innerHTML = `<button title="přepsat tenhle odstavec" data-pt="edit">✏️</button>`;
        ...
        gap.title = 'přidat text nebo diagram sem';
  ...
  async submitRemix(blockId) {
    ...
    try {
      ...
      if (status) status.innerHTML = `<span style="color:var(--warn,#D97706);font-size:.72rem">Remix se nepovedl (${this.escHtml(String(e.message).slice(0, 120))}). Tvoje přání jsme si zapsali.</span>`;
  ...
  _diagramRemixBtn(block) {
    ...
    return `<button class="diagram-remix-btn" onclick="app.remixDiagram('${block.id}')" title="✨ Remixni tenhle diagram — popiš, co se má změnit">✨</button>`;
  }
  ...
  async remixDiagram(blockId) {
    ...
    box.className = 'share-consent remix-form';
    if (!canGen) {
      box.innerHTML = `<b>✨ Remix diagramu</b><div style="font-size:.72rem;color:var(--text-3);margin-top:.3em">Generování teď není zapnuté — tvoje přání jsme zapsali pro redakci.</div>`;
      this.rc.logEvent('remix_miss', { blockId, diagram: true });
    } else if (this.user.readerMode !== 'open') {
      box.innerHTML = `<b>✨ Remix diagramu</b><div style="font-size:.75rem;margin-top:.3em">Remixem si vytvoříš vlastní soukromou verzi této části. Nejdřív si zapni <b>otevřený režim</b> v <a href="#" onclick="app.switchView('profile');return false">Profilu</a>.</div>`;
    } else {
      box.innerHTML = `
        <b>✨ Remixnout tenhle diagram${el.querySelector('animate, animateTransform, animateMotion') ? ' / animaci' : ''}</b>
        <textarea id="remix-prompt-${blockId}" rows="2" placeholder="Co se má změnit? např. 'překresli to s telefony místo počítačů', 'zpomal animaci', 'přidej do příkladu třetí zařízení'"
  ...
  async submitDiagramRemix(blockId) {
    ...
    try {
      ...
      if (!srcSvg) throw new Error('u této části není žádné SVG, které by šlo remixovat');
      if (srcSvg.length > 60000) throw new Error('tenhle diagram je na remix (zatím) moc složitý');
      ...
      if (status) status.innerHTML = `<span style="color:var(--warn,#D97706);font-size:.72rem">Remix se nepovedl (${this.escHtml(String(e.message).slice(0, 140))}). Tvoje přání jsme si zapsali.</span>`;
  ...
  _showShareConsent(blockId) {
    ...
    box.innerHTML = `
      <b>Super, že se ti líbí!</b> Chceš tohle podání nasdílet do knihy, aby ho objevili i další čtenáři s podobným nastavením?
      <div style="font-size:.7rem;color:var(--text-3);margin:.3em 0">Sdílí se anonymně, pokud nechceš jinak. Dokud ho neprověří redakce, zůstane označené jako čtenářské. Můžeš si ho nechat i soukromé — tvoje bude tak jako tak.</div>
      <input type="text" id="share-nick-${blockId}" placeholder="Nepovinná přezdívka (prázdné = anonymně)" maxlength="40"
      ...
      <div class="note-actions">
        <button class="note-save" onclick="app.shareGeneratedBlock('${blockId}')">📣 Sdílet do knihy</button>
        <button class="note-cancel" onclick="document.getElementById('share-consent-${blockId}').remove()">Nechat soukromé</button>
      </div>`;
  ...
  async shareGeneratedBlock(blockId) {
    ...
    if (ok) {
      ...
      this.showXPToast('+10 XP 📖 Obohatil/a jsi knihu!', 'achievement');
      if (this._f('gamification')) { this.user.addXP(10); this.user.save(); this.updateXPBadge(); }
    } else {
      this.showXPToast('Sdílení se nepovedlo — zůstává soukromé, požadavek zapsán', 'xp');
    }
  }
  ...
  saveBlock(blockId) {
    ...
    if (!this.user.seenBlocks.has(blockId)) this.user.trackSeen(blockId);
    const btn = document.querySelector(`#b-${blockId} .act-btn[title="Uložit na později"]`);
  ...
  _updateMissionBar() {
    ...
    const step = this._wizardStep;
    container.innerHTML = `
      <button class="mission-bar-back" onclick="app._returnToWizard()" title="Další krok">&#127919;</button>
    ...
    if (auth) {
      el.innerHTML = `<label>Účet</label>
        ...
        <div id="editAccountForm" style="display:none;margin-top:.4em">
          ...
          <input type="password" id="editPassword" class="auth-input" placeholder="Nové heslo (prázdné = beze změny)" maxlength="100" style="font-size:.78rem;padding:.4em .6em">
          <div class="auth-btns">
      ...
      el.innerHTML = `<label>Účet</label>
        <div style="margin-top:.3em">
          ...
          <input type="password" id="authPassword" class="auth-input" placeholder="Heslo (aspoň 4 znaky)" maxlength="100" style="font-size:.78rem;padding:.4em .6em">
          <div class="auth-btns">
  ...
  async updateAccount() {
    ...
    if (password) {
      if (password.length < 4) { if (errEl) errEl.textContent = 'Heslo musí mít aspoň 4 znaky.'; return; }
      body.password = password;
    }
    if (!name && !password) { if (errEl) errEl.textContent = 'Není co uložit.'; return; }
    if (errEl) errEl.textContent = 'Ukládám…';
  ...
  _renderSettingsThemes() {
    ...
    rewards.forEach(r => {
      ...
      h += `<button class="cosmetic-item ${unlocked ? '' : 'cosmetic-locked'} ${active ? 'cosmetic-active' : ''}" ${unlocked ? `onclick="${onclick}"` : ''}>
        ...
        <span class="cosmetic-level">${unlocked ? (active ? '\u2713' : '') : 'úr. ' + r.level}</span>
      </button>`;
    });
    h += '</div>';
    el.innerHTML = h;
  ...
  _renderActivityHeatmap() {
    ...
    days.forEach((d, i) => {
      ...
      svg += `<rect x="${x}" y="${y}" width="${cellSize}" height="${cellSize}" rx="2" fill="${fill}"><title>${d.key}: interakcí: ${d.count}</title></rect>`;
    });
    svg += '</svg>';

    let h = '<div class="profile-section"><h3>Tvoje aktivita</h3>';
    h += `<div style="display:flex;gap:1em;margin-bottom:.6em;font-size:.78rem">`;
    h += `<span style="color:var(--text-3)">aktivních dnů: ${totalActive}</span>`;
    h += `<span style="color:var(--text-3)">interakcí: ${totalInteractions}</span>`;
    h += `</div>`;
    h += svg;
  ...
  getLevelRewards() {
    return [
      { level: 1, name: 'Výchozí', theme: null, icon: '\u{1F331}' },
      { level: 2, name: 'Oceán', theme: 'ocean', icon: '\u{1F30A}' },
      { level: 3, name: 'Západ slunce', theme: 'sunset', icon: '\u{1F305}' },
      { level: 4, name: 'Les', theme: 'forest', icon: '\u{1F332}' },
      { level: 5, name: 'Galaxie', theme: 'galaxy', icon: '\u{1F30C}' },
      { level: 7, name: 'Neon', theme: 'neon', icon: '\u{1F4A1}' },
      { level: 10, name: 'Zlato', theme: 'gold', icon: '\u{1F451}' },
    ];
  }
  ...
  checkGamificationEvents() {
    ...
    if (this.user._pendingLevelUp) {
      this.showXPToast('Úroveň ' + this.user._pendingLevelUp + '! Teď jsi: ' + this.user.getLevelTitle(), 'levelup');
  ...
  proposeConcept(seed) {
    const text = (window.prompt('🌱 Jaký koncept v téhle knize chybí?\n\nPopiš ho jednou dvěma větami — každý návrh projde redakcí (a pomocnou AI).', seed || '') || '').trim();
    ...
    if (this._f('gamification')) { this.user.addXP(10); this.user.save(); this.updateXPBadge(); }
    this.showXPToast('🌱 +10 XP — návrh konceptu zapsán pro redakci', 'achievement');
  }
  ...
  async shareThing(title, text, url) {
    ...
    try {
      ...
      this.showXPToast('🔗 Odkaz zkopírován — pošli ho kamarádovi', 'achievement');
  ...
  _ghostCardHtml(p, ctx) {
    ...
    return `<div class="card ghost-card" style="flex:0 0 262px;border-top:3px solid #0EA5E9">
      ...
      <div style="font-size:.64rem;color:var(--text-3);font-style:italic;margin:.3em 0">Uměl/a bys pak odpovědět: ${this.escHtml(p.recallQ)}</div>
  ...
  ghostVote(slug, v, ctx) {
    ...
    if (el) {
      el.innerHTML = v > 0
        ? `<span style="font-size:.7rem;color:#0EA5E9">✓ +2 XP — zájem zapsán, redakce vidí souhrny. <a href="#" onclick="event.preventDefault();app.proposeConcept('${this.escHtml(slug)}: ')" style="color:var(--accent)">přidej svůj pohled</a></span>`
        : `<span style="font-size:.7rem;color:var(--text-3)">✓ zapsáno — tenhle návrh už neuvidíš</span>`;
    }
  }
  ...
  _checkAdoptions() {
    ...
    for (const b of Object.values(this.privateBlocks || {})) {
      ...
      if ((b.meta.sharedAt || b.meta.state === 'community') && this.findBlock(id) && !awarded.has(id)) {
        ...
        setTimeout(() => this.showXPToast('📖 Tvoje podání bylo přijato do knihy! +100 XP — redaktorská dráha', 'achievement'), 1200);
      }
    }
    if (changed) {
  ...
  getCertTiers() {
    ...
    const tiers = [
      { id: 'foundations', icon: '\u{1F949}', title: 'Základy', subtitle: 'rozumí tomu, jak funguje internet', color: '#B45309',
        reqs: [{ label: `přečtený základ (${readCount(base)}/${base.length})`, done: readAll(base) }] },
      { id: 'practitioner', icon: '\u{1F948}', title: 'Praktik', subtitle: 'umí síť vysvětlit i prozkoumat', color: '#64748B',
        reqs: [
          { label: `přečtený technický základ (${readCount(tech)}/${tech.length})`, done: readAll(tech) },
          { label: `splněné mise (${Math.min(done.length, 2)}/2)`, done: done.length >= 2 },
        ] },
      { id: 'guru', icon: '\u{1F947}', title: 'Guru internetu', subtitle: 'mistr sítí a digitálního světa', color: '#CA8A04',
        reqs: [
          { label: `přečtený výzkumný základ (${readCount(research)}/${research.length})`, done: readAll(research) },
          { label: `poražený boss pokročilé mise (${Math.min(advancedDone, 1)}/1)`, done: advancedDone >= 1 },
          { label: `opakování kartiček (${Math.min(recallReps, 10)}/10)`, done: recallReps >= 10 },
        ] },
    ];
    let achievedIdx = -1;
  ...
  showCertificateModal() {
    ...
    overlay.innerHTML = `
      <div class="cert-modal">
        ...
        <p style="font-size:.82rem;color:var(--text-2);margin-bottom:1em">Gratulujeme k dokončení knihy! Vyplň údaje a certifikát bude jen tvůj.</p>
        <label class="cert-label">Tvoje jméno <span style="color:var(--accent)">*</span></label>
        <input type="text" id="certName" class="cert-input" placeholder="např. Jana Nováková" value="${this.escHtml(savedName)}" maxlength="60">
        <label class="cert-label">Jak se ti kniha líbila? <span style="font-weight:400;color:var(--text-3)">(nepovinné)</span></label>
        <textarea id="certComment" class="cert-input" rows="3" placeholder="Dozvěděl/a jsem se, že internet funguje…" maxlength="500"></textarea>
        <label class="cert-label">Doporučil/a bys tuhle knihu? <span style="font-weight:400;color:var(--text-3)">(nepovinné)</span></label>
        ...
        <input type="email" id="certEmail" class="cert-input" placeholder="tvuj@email.cz" value="${this.escHtml(savedEmail)}" maxlength="120">
  ...
  _buildCertSVG(name) {
    const u = this.user;
    const p = u.getProfile(this.allBlocks);
    const date = new Date().toLocaleDateString('cs-CZ', { year: 'numeric', month: 'long', day: 'numeric' });
    // Specialization line from the facet profile (voice is legacy)
    const _tf = u.getTargetFacets();
    const voiceLabel = [
      _tf.depth && _tf.depth !== 'standard' ? `hloubka ${_tf.depth}` : null,
      _tf.lens && _tf.lens !== 'generic' ? `svět ${_tf.lens}` : null,
      _tf.lang === 'cs' ? 'česká edice' : null,
    ].filter(Boolean).join(' · ') || 'univerzální';
    ...
    const tierHeader = achievedIdx >= 0 ? `CERTIFIKÁT • ${tier.title.toUpperCase()}` : 'CERTIFIKÁT O DOKONČENÍ';
    const tierLine = achievedIdx >= 0 ? esc(`${tier.subtitle} — ověřený základ knihy v ${tier.id === 'guru' ? 'výzkumné' : tier.id === 'practitioner' ? 'technické' : 'základní'} hloubce`) : 'úspěšně prošel/prošla knihou Jak funguje internet?';
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 566" width="800" height="566">
  ...
  <!-- Title -->
  <text x="400" y="140" text-anchor="middle" font-family="Georgia,serif" font-size="28" fill="#4C1D95" font-weight="bold">Jak funguje internet?</text>
  <text x="400" y="168" text-anchor="middle" font-family="system-ui,sans-serif" font-size="13" fill="#7C3AED">P-book k lekcím Internet4Kids</text>
  ...
  <text x="200" y="335" text-anchor="middle" font-family="system-ui,sans-serif" font-size="11" fill="#7C3AED" font-weight="600">přečteno částí: ${p.progress.read}</text>
  ...
  <text x="600" y="335" text-anchor="middle" font-family="system-ui,sans-serif" font-size="11" fill="#7C3AED" font-weight="600">získaných odznaků: ${u.achievements.length}</text>

  <text x="200" y="358" text-anchor="middle" font-family="system-ui,sans-serif" font-size="10" fill="#9CA3AF">dokončeno ${p.progress.pct} %</text>
  ...
  <text x="400" y="398" text-anchor="middle" font-family="system-ui,sans-serif" font-size="12" fill="#7C3AED" font-weight="600">Zaměření: ${voiceLabel}</text>

  <!-- Topics -->
  <text x="400" y="438" text-anchor="middle" font-family="system-ui,sans-serif" font-size="10" fill="#9CA3AF">Klíčová témata: ${(p.topTopics || []).slice(0, 4).join(' \u2022 ') || 'Jak funguje internet'}</text>

  <!-- Footer -->
  <line x1="150" y1="470" x2="350" y2="470" stroke="#D4B5FD" stroke-width="1"/>
  <text x="250" y="488" text-anchor="middle" font-family="Georgia,serif" font-size="12" fill="#4C1D95">Tým Internet4Kids</text>
  <text x="250" y="502" text-anchor="middle" font-family="system-ui,sans-serif" font-size="9" fill="#9CA3AF">MFF UK &amp; PedF UK</text>
  ...
  generateCertificate() {
    ...
    if (!u.achievements.find(a => a.id === 'certified')) {
      u.achievements.push({ id: 'certified', name: 'S certifikátem!', icon: '\u{1F393}', desc: 'Získal/a jsi certifikát', earnedAt: Date.now() });
      u.addXP(50);
      u.save();
    ...
    if (preview) {
      preview.innerHTML = `<div style="border:1px solid var(--border);border-radius:8px;overflow:hidden;margin-bottom:.5em">${svg.replace('<?xml version="1.0" encoding="UTF-8"?>', '')}</div>
        <p style="font-size:.75rem;color:var(--product);font-weight:600">Certifikát stažen! Níže ho můžeš i sdílet.</p>`;
    }
  }
    ...
    try {
      ...
      if (navigator.share && navigator.canShare?.({ files: [new File([pngBlob], 'certificate.png', { type: 'image/png' })] })) {
        await navigator.share({
          title: 'Dokončil/a jsem knihu „Jak funguje internet?“!',
          text: `Právě jsem dočetl/a p-book „Jak funguje internet?“ (Internet4Kids) a mám certifikát!`,
          files: [new File([pngBlob], 'recsys-certificate.png', { type: 'image/png' })]
        });
      } else {
        // Fallback: copy PNG to clipboard
        await navigator.clipboard.write([new ClipboardItem({ 'image/png': pngBlob })]);
        this.showXPToast('Certifikát zkopírován do schránky! Vlož ho kamkoliv a sdílej.', 'info');
      }
    } catch (e) {
      ...
      a.click();
      URL.revokeObjectURL(a.href);
      this.showXPToast('Certifikát uložen! Pochlub se kamarádům.', 'info');
    }
  }

  resetAll() {
    if (!confirm('Vynulovat všechna data o čtení? Smaže se tvůj postup, poznámky i předvolby.')) return;
    this.user.reset();
  ...
  getChapterLabel(block) {
    for (const [i, ch] of Object.entries(this.chapters)) {
      if (ch.blocks.some(b => b.id === block.id)) return `Kap. ${ch.number}: ${ch.title}`;
    }
    return '';
  }
}
...
function _showHighlightPopup() {
  ...
  if (last) last.title = tiny ? 'Přidat sem nový text nebo diagram' : 'Vylepšit tuhle pasáž — uprav ji sám, nebo ji nech přepsat AI';
  const range = sel.getRangeAt(0);
